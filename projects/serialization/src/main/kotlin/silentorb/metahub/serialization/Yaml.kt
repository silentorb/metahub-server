...
inline fun <reified T> parseYaml(yaml: String?): T =
    getYamlObjectMapper().readValue(yaml, T::class.java)
...
fun <T> saveYaml(mapper: YAMLMapper, path: String, config: T) {
  Files.newBufferedWriter(Paths.get(path)).use {
    mapper.writeValue(it, config)
  }
}

fun <T> saveYaml(path: String, config: T) {
  saveYaml(getYamlObjectMapper(), path, config)
}
